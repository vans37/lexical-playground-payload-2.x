...
import useLayoutEffect from '../../../lib/shared/useLayoutEffect'

import type { Comment, Comments, Thread } from '../../../lib/commenting'
import {
  CommentStore,
  createComment,
  createThread,
...
import CommentEditorTheme from '../../../lib/CommentEditorTheme'
import Button from '../../../lib/ui/Button'
import ContentEditable from '../../../lib/ui/ContentEditable'
...
import {
  getCommentsFromCollection,
  prepareCommentsCollection,
  saveComments,
  useCollabAuthorName,
} from './comments'
...
export default function CommentPlugin(): JSX.Element {
  ...
  //comments
  const { collection, id } = useDocumentInfo()

  const hash = useMemo(() => {
    if (id) {
      const combined = collection.slug + id.toString()
      const hash = cyrb53(combined)
  ...
  useEffect(() => {
    const setUpComments = async () => {
      if (collection?.slug) {
        if (hash) {
          await prepareCommentsCollection(hash)
          const commentsSerialized = await getCommentsFromCollection(hash)
          if (commentsSerialized || commentsSerialized.length > 0) {
            const parsedCommentsArr: Comment[] = JSON.parse(commentsSerialized)
            commentStore.setComments(parsedCommentsArr)
            triggerOnChange(commentStore)
          }
        }
      }
    }
    void setUpComments()
  }, [collection?.slug, commentStore, hash])
  ...
  const deleteCommentOrThread = useCallback(
    (comment: Comment | Thread, thread?: Thread) => {
      if (comment.type === 'comment') {
        ...
        saveComments(commentStore, hash)
      } else {
    ...
    [commentStore, editor, hash, markNodeMap],
  )
